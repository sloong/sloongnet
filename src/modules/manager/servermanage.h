...
struct TemplateItem
{
    ...
    void BuildCache() { ConfigurationObj = ConvertStrToObj<GLOBAL_CONFIG>(this->Configuration); }
    int ID;
    string Name;
    string Note;
    int Replicas;
...
class CServerManage : public IObject
{
  ...
  private:
    ...
    bool CheckTemplateIsRegistered(int);
    bool CheckTemplateNeedCreate(int, bool);
