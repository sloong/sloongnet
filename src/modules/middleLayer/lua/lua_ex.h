...
class UNIVERSAL_API CLua
{
  public:
    ...
    inline lua_State *GetScriptContext()
    {
    ...
    inline vector<string> *GetSearchRouteList()
    {
        return &m_listSearchRoute;
    }

    // Push value
    void PushPacket(CLuaPacket *pData);
    bool PushFunction(int nFuncRef);
    inline void PushString(const string &strString)
    {
        PushString(m_pScriptContext, strString);
    }
    inline void PushDouble(double dValue)
    {
        CLua::PushDouble(m_pScriptContext, dValue);
    }
    inline void PushInteger(int nValue)
    {
        PushInteger(m_pScriptContext, nValue);
    }
    inline void PushPointer(void *pPointer)
    {
    ...
    CResult RunFunction(const string &, CLuaPacket *, int = 0, const string & = "", const string & = "",
                        string *extendDataUUID = nullptr);
    inline CResult RunFunction(const string &strFunctionName, const string &args)
    {
        return RunString(format("%s(%s)", strFunctionName.c_str(), args.c_str()));
    }
    CResult RunEventFunction(const string &, int, const string &);

    // Get Valkue
    inline string GetString(int nNum, const string &strDefault = "")
    {
        return GetString(m_pScriptContext, nNum, strDefault);
    }
    double GetDouble(int nNum, double dDefault = -1.0f)
    {
        return GetDouble(m_pScriptContext, nNum, dDefault);
    }
    inline int GetInteger(int nNum, int nDef = -1)
    {
        return GetInteger(m_pScriptContext, nNum, nDef);
    }
    inline bool GetBoolen(int nNum)
    {
        return GetBoolen(m_pScriptContext, nNum);
    }
    inline void *GetPointer(int nNum)
    {
        return GetPointer(m_pScriptContext, nNum);
    }
    inline unique_ptr<map<string, string>> GetTableParam(int index)
    {
        return GetTableParam(m_pScriptContext, index);
    }

    inline void EnableLog(spdlog::logger *p)
    {
        m_pLog = p;
    }
  ...
  protected:
    ...
    {
        auto str = format("\n [{}]Error - {}:\n {}\n Error Message:{}", res, strErrorTitle,
                          strErrorOperation, GetString(m_pScriptContext, 1));
// format("\n Error - %s:\n %s\n Error Message:%s", strErrorType.c_str(), strCmd,
// GetCallStack(m_pScriptContext).c_str())
