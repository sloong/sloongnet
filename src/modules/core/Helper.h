 ...
 * @Description: The helper module provides some inline functions and template function to help easily coding.
 */
/***
 * @......................................&&.........................
 * @....................................&&&..........................
...
template <typename T, typename K> inline T STATIC_TRANS(K p)
{
...
template <typename T, typename K> inline auto DYNAMIC_TRANS(K p)
{
...
template <class T> inline shared_ptr<T> ConvertStrToObj(const string &obj)
{
...
inline void PrintPackage(spdlog::logger *log, Package *package, const string &title,
                         spdlog::level::level_enum level = spdlog::level::level_enum::trace)
{
    ...
    else
    {
        log->log(level, title + format("Function: %d Priority: %d ID: %llu Content[L]: %d Extend[L]: %d",
                                       package->function(), package->priority(), package->id(),
                                       package->content().length(), package->extend().length()));
    }
}
...
inline int GetJsonInt(const Json::Value &j, const string &key, bool allowString = true)
{
    if (j.isMember(key))
    {
        ...
        if (!allowString)
            std::throw_with_nested(invalid_argument("Json: value not int type."));
        if (!v.isString())
            std::throw_with_nested(invalid_argument("Json: value not int/string type."));
        int res = 0;
