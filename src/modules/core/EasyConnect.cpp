...
CResult Sloong::EasyConnect::InitializeAsServer(spdlog::logger *log, SOCKET sock, LPVOID ctx)
{
    ...
    m_nHashCode = std::hash<string>{}(format("{}:{}", m_strAddress, m_nPort));
...
CResult Sloong::EasyConnect::Connect()
{
    ...
    if (m_pOnReconnect.size() > 0)
        for_each(m_pOnReconnect.begin(), m_pOnReconnect.end(),
                 [&](auto func) { func(m_nHashCode, m_nInvalidSocket, m_nSocket); });
    return CResult::Succeed;
}
...
CResult Sloong::EasyConnect::SendPackage(UniquePackage pack)
{
    ...
    else if (!m_strSending.empty() && pack != nullptr)
    {
        return CResult::Make_Error(
            "Prev package is no sending done! Make sure the send status is checked before sending new package.");
    }
    ...
    if (m_SentSize == 0)
    {
        ...
        {
            auto msg = format("Error when send length data. Socket[{}][{}:{}].Errno[{}].", m_nSocket, m_strAddress,
                              m_nPort, GetErrno());
            Close();
    ...
    {
        auto msg =
            format("Error when send data. Socket[{}][{}:{}].Errno[{}].", m_nSocket, m_strAddress, m_nPort, GetErrno());
        Close();
        return CResult::Make_Error(msg);
    }
}
...
PackageResult Sloong::EasyConnect::RecvPackage(bool block)
{
    ...
    if (len > 0)
    {
        ...
        {
            if (m_pLog)
                m_pLog->debug(format("Receive package returned [Retry]. Package size[{}], Received[{}][{}]",
                                     m_RecvPackageSize, m_ReceivedSize, m_strReceiving.length()));
    ...
    {
        if (m_pLog)
            m_pLog->debug(format("Receive package returned [Retry]. Package size[{}], Received[{}]", m_RecvPackageSize,
                                 m_ReceivedSize));
        return PackageResult(ResultType::Retry);
    }
    else
    {
        Close();
        return PackageResult::Make_Error(format("Error when receive data. Socket[{}][{}:{}].Errno[{}].", m_nSocket,
                                                m_strAddress, m_nPort, GetErrno()));
    }
}
