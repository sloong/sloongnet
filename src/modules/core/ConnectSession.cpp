...
ResultType Sloong::ConnectSession::SendDataPackage(UniquePackage pack)
{
    ...
    m_pLog->trace(
        format("SEND>>>[{}]>>No[{}]>>[{}]byte", m_pConnection->GetSocketID(), pack->id(), pack->ByteSizeLongEx()));

    // if have exdata, directly add to epoll list.
    if (IsBigPackage(pack.get()) || m_pConnection->IsSending() ||
        (m_bIsSendListEmpty == false && !m_oPrepareSendList.empty()) || m_oSockSendMutex.try_lock() == false)
    {
    ...
    {
        m_pLog->trace(format("Send data done. wait next send time.[{}/{}]", m_pConnection->m_SendPackageSize,
                             m_pConnection->m_SentSize));
        return ResultType::Retry;
    }
    else
    {
        m_pLog->error(
            format("Unintended result[{}] in SendDataPackage in SockInfo.", ResultType_Name(res.GetResult())));
        return ResultType::Error;
    }
}
...
ReceivePackageListResult Sloong::ConnectSession::OnDataCanReceive()
{
    ...
    do
    {
        ...
        if (res.GetResult() == ResultType::Succeed)
        {
            ...
            {
                m_pLog->trace(format("RECV<<<[{}]<<No[{}]<<[{}]byte", m_pConnection->GetSocketID(), package->id(),
                                     package->ByteSizeLong()));
                ...
                {
                    auto msg =
                        format("Hash check error. Package[{}]<->[{}]Calculate", ConvertToHexString(hash.c_str(), 0, 31),
                               ConvertToHexString((char *)buffer, 0, 31));
                    m_pLog->warn(msg);
                    AddToSendList(Package::MakeErrorResponse(package.get(), msg));
                    continue;
                }
        ...
        {
            m_pLog->error(
                format("Unintended result[{}].Loop[{}].", ResultType_Name(res.GetResult()), bLoop ? "true" : "false"));
            break;
        }
    } while (bLoop);
...
void Sloong::ConnectSession::ProcessPrepareSendList()
{
    ...
    while (m_oPrepareSendList.take(&pack))
    {
        ...
        m_pLog->debug(format("Add package to send list[{}]. send list size[{}], prepare send list size[{}]", priority,
                             m_pSendList[priority].size(), m_oPrepareSendList.size()));
    }
}
...
ResultType Sloong::ConnectSession::ProcessSendList()
{
    ...
    {
        m_pLog->debug(format("Start send package : AllSize[{}],Sent[{}]", m_pConnection->m_SendPackageSize,
                             m_pConnection->m_SentSize));
        ...
        {
            m_pLog->error(format("Send data package error. close connect:[{}:{}]", m_pConnection->m_strAddress,
                                 m_pConnection->m_nPort));
            return ResultType::Error;
        }
        else if (res.GetResult() == ResultType::Retry)
        {
            m_pLog->debug(
                format("Send data package done but not all data is send All[{}]:Sent[{}]. wait next write sign.",
                       m_pConnection->m_SendPackageSize, m_pConnection->m_SentSize));
            return ResultType::Retry;
        }
        else
        {
            m_pLog->error(
                format("Send data package returned Unexpected results {}.", ResultType_Name(res.GetResult())));
            return ResultType::Retry;
        }
    }
