 ...
 * @LastEditTime: 2021-10-20 14:08:12
...
void PrientHelp()
{
    ...
    cout << "--include=<TypeName>: Set it for target module type. If have multi type, use ',' to split. If assign is "
            "setted, this will be ignored."
         << endl;
    cout << "--exclude=<TypeName>: Set it for target is not support type. If have multi type, use ',' to split. If "
            "assign/include is setted, this will be ignored."
         << endl;
...
int main(int argc, char** args)
{
    ...
    try
    {
        auto logger = make_shared<logger_ex>(spdlog::stdout_color_mt("MAIN"));
        ...
        do
        {
            ...
            res = Sloong::CSloongBaseService::Instance->Initialize(info, logger);
            if (!res.IsSucceed())
            {
            ...
            logger->info(format("Base service instance is end with result {}. Message {}",
                                ResultType_Name(res.GetResult()), res.GetMessage()));
