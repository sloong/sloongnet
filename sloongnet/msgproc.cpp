...
{
	pLua->RunScript(m_stLuaConfig->EntryFile);
    // get current path
    ...
    string strDir(szDir);
    strDir += "/" + path;
	pLua->RunFunction(m_stLuaConfig->EntryFunction, CUniversal::Format("'%s'", strDir));
}
...
int CMsgProc::MsgProcess( int id, CLuaPacket* pUInfo, string& msg, string&res, char*& pBuf)
{
	...
	{
		InitLua(pLua, m_stLuaConfig->ScriptFolder);
		m_pGFunc->m_pReloadTagList[id] = false;
	}

	int nRes = pLua->RunFunction(m_stLuaConfig->ProcessFunction, pUInfo, msg, res);
...
{
	CLua* pLua = new CLua();
	pLua->SetScriptFolder(m_stLuaConfig->ScriptFolder);
