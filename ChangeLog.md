...
***
### v0.9.0
* 新增：[#55 ](https://git.sloong.com/public/sloongnet/issues/55)支持SSL连接
* 新增：[#34 ](https://git.sloong.com/public/sloongnet/issues/34)支持Log网络输出的端口设置
* 优化：系统框架进行优化重构，重构后的性能关键指标为2个：1.消息处理线程数，这将影响到消息分发的速度。（只是分发并不负责处理）
2.处理线程池的大小。处理分发后的消息，消息队列越大则此处线程池应同步增大。
* 修改：移除database的支持，后续将以独立扩展库的形式单独发布。

### v0.8.1
* 功能改进： [#51 ](https://git.sloong.com/public/sloongnet/issues/51)调整流水号和优先级字段的格式为数字型
* 修复：[#53 ](https://git.sloong.com/public/sloongnet/issues/53)频繁触发Epoll发送函数但实际没有数据需要发送的问题
* 新增：[#48 ](https://git.sloong.com/public/sloongnet/issues/48)添加接收超时支持
* 新增：[#52 ](https://git.sloong.com/public/sloongnet/issues/52)支持Log级别的设置
